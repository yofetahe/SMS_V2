...
<%-- 	<c:set var="cd_id" value="cdetail_bean.cd_id"/> --%>
	...
	<div id="studwithid" class="content_background">
		...
		<table id="stud_with_id_rslt" class="table table-striped table-bordered">
			<thead>
				<tr>
					<th width="5%">No</th>
					<th>Student Name</th>
			...
			<tbody>
				<c:forEach varStatus="stat" var="stud_with_id_rslt" items="${stud_with_id_rslt}">
					<tr>
						<td align="center">
						...
						<td>
							<div style="color: #3d6e9f;">
								<c:out value="${stud_with_id_rslt[1]}"/> <c:out value="${stud_with_id_rslt[2]}"/> <c:out value="${stud_with_id_rslt[3]}"/>
							</div>									
						</td>
						<td align="left">
							<div>
								<c:out value="${stud_with_id_rslt[9]}"/>
							</div>
						</td>			
					</tr>
				</c:forEach>
